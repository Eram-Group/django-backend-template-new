...
`just test` runs pytest on a fresh test database every time (no
`--reuse-db`: a kept database drifts from the models and accumulates
session-seeded rows); CI adds `--cov` with a hard
...
- Results are rows: prune with `manage.py prune_db_task_results
  --min-age-days 14` (by hand; nothing runs on a timer).
...
architecture, cost, one-time bootstrap, deploy flow, operations —
