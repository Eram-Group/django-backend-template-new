...
#: ``A,B,C`` in the environment -> ``["A", "B", "C"]``; the split rides on the
#: alias, so a new list field can never forget it.
type CommaSeparated[T] = Annotated[
    list[T], NoDecode, BeforeValidator(_split_comma_separated)
]
...
class Env(BaseSettings):
    ...
    WHATSAPP_WEBHOOK_VERIFY_TOKEN: SecretStr | None = None  # GET handshake echo guard
