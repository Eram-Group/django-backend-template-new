...
class EnvConfig:
    ...
    web_memory: int
    web_max_tasks: int
    worker_spot: bool  # Fargate Spot (~70% off); every env by default, see production
    database: DatabaseKind
    custom_domain: str | None = None
    # Express Mode scaling: floor and the average-CPU target it scales on.
    web_min_tasks: int = 1
    web_cpu_target: int = 60
    # CloudWatch retention of the web/worker log groups (a RetentionDays name).
    log_retention: Literal["ONE_WEEK", "ONE_MONTH", "THREE_MONTHS", "ONE_YEAR"] = (
        "ONE_MONTH"
    )
    worker_cpu: int = 256
...
ENV_SECRET: frozenset[str] = frozenset(
    {
        # Admin path: unguessable, chosen per environment by a human (local: admin/).
        "ADMIN_URL",
        "SECRET_KEY",
        "DATABASE_URL",
...
ENVIRONMENTS: dict[EnvName, EnvConfig] = {
    ...
    "production": EnvConfig(
        ...
        web_memory=1024,
        web_max_tasks=6,
        # Spot in production too: the worker is a queue consumer whose jobs
        # are idempotent and commit-or-roll-back with the DB, and db_worker
        # finishes the current job on SIGTERM - an interruption delays work by
        # minutes and never loses it. On-demand is for a worker running long,
