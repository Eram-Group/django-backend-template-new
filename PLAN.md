...
10. CI/CD: `.github/workflows/ci.yml` (hybrid: fast uv job + parallel image-smoke job per Stack table), secret-scan workflow (TruffleHog), `deploy-dev.yml` (main: OIDC ECR push `github.sha` → dev ECS deploy → Apidog sync), `deploy-prod.yml` (dispatch: bump-input version → promote → deploy → tag + GitHub Release); `renovate.json`.
