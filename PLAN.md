...
| Tasks | `django.tasks` + `django-tasks-db` backend | Queue in Postgres; worker = `manage.py db_worker` as 2nd ECS service (same image, different command). Enqueue inside the service transaction. Recurring commands (`clearsessions`, `prune_db_task_results`, `reconcile_payments`, `sweep_deliveries`) are not scheduled (EventBridge Scheduler removed 2026-09-05 pending a scheduling decision). Escape hatch: swap backend, call sites unchanged. |
