...
| Outbound clients (built 2026-07-12) | httpx + stamina (retries) + respx (test mocks); firebase-admin (FCM HTTP v1); django-phonenumber-field | One kernel `apps/common/http.py::request_json` (explicit timeouts, typed retry policies, error taxonomy, structlog). Transport switch = EMAIL_BACKEND pattern: `SMS_BACKEND` (OurSMS SA / SMSMisr EG behind country routing), `PUSH_BACKEND` (own Device model — NOT fcm-django, it drags DRF), `PAYMENT_GATEWAYS` currency map (Tap SAR / Paymob EGP in every environment — env keys pick test vs live mode, HMAC-verified webhooks, Decimal money, wallet ledger via `wallet_apply` row lock; saved cards 2026-07-18: `SavedCard` token vault — PCI stays at the provider — with opt-in save, one-click CIT, service-only MIT via Tap payment-agreement non-3DS / Paymob CoF + MOTO, Paymob's TOKEN callback verified with its own 8-field HMAC, and add-card-without-payment `POST /cards/add` — a nominal CARD_VERIFICATION row, Tap authorize+auto-void / Paymob Verification integration, no wallet credit). SMS/push: console locally, real transports only when deployed; tests: locmem outboxes + FakeGateway pinned in test.py. |
