...
| Config | pydantic-settings `Env` in `config/env.py` + dj-database-url (parser only) | `Env` owns every `.env` field: typed (`SecretStr` secrets, `Literal` environment), fail-fast at startup, loads `.env`. dj-database-url only converts DB URL → `DATABASES` dict. Settings split: `config/settings/{base,local,production,test}.py` consume `env`. |
