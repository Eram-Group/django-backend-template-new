...
``execute_deliveries`` (the body of the ``deliver_notifications`` task) takes
a list of delivery pks (a single send passes its 1-3 rows; broadcast
batches pass channel-pure rows). The executor claims PENDING rows atomically
(select_for_update skip_locked -> PROCESSING), so a double-enqueued or
re-run task can never double-send - the unclaimed rows are simply skipped.
...
PROCESSING. Each channel's rows, its broadcast counters and the completion
probe commit in ONE transaction, so a crash can never leave rows terminal
with the counters behind.
...
def _record_outcomes(rows: list[NotificationDelivery]) -> None:
    ...
    while they exist; the sweep resets exactly those. Every claimed row was
    PENDING (never counted) when this batch took it - a FAILED row that
    ``deliveries_resume`` re-queued was uncounted at that moment - so each
    terminal outcome here is one +1, never a second count of the same row.
    """
    now = timezone.now()
    for row in rows:
    ...
    with transaction.atomic():
        ...
        for broadcast_id, counts in per_broadcast.items():
            broadcast_record_progress(broadcast_id=broadcast_id, **counts)
