...
def test_to_minor_units(amount: Decimal, expected: int) -> None:
    assert to_minor_units(amount=amount, currency="SAR") == expected
    assert to_minor_units(amount=amount, currency="EGP") == expected


def test_to_minor_units_rejects_sub_cent_precision() -> None:
    with pytest.raises(ValueError, match="decimal places"):
        to_minor_units(amount=Decimal("1.001"), currency="SAR")


def test_to_minor_units_never_floats() -> None:
    # The classic float bug: 19.99 * 100 == 1998.9999...
    assert to_minor_units(amount=Decimal("19.99"), currency="EGP") == 1999
